...
import com.kujilabo.config.elasticsearch.ElasticSearchConstants
...
class EsDocumentBuilderOld extends LazyLogging {
  ...
  def getHighlightValue(key: String, value: String,
    ...
    if (highlightList.contains(key)) {
      ...
      if (value.length() > ElasticSearchConstants.HIT_RESULT_CONTENT_MAX_LENGTH) {
        value.substring(0, ElasticSearchConstants.HIT_RESULT_CONTENT_MAX_LENGTH)
      }
      else {
        value
      }
